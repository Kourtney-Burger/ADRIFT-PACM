...
## Subset Project
```{r}
detections <- detections[str_detect(detections$DriftName, "PASCAL"),]
```
...
```{r}
# Read in metadata
metadata <- read.csv(here::here('PACM Worksheets/PASCAL/SWFSC_20240520_METADATA.csv'))
...
#Exclude NBHF unitl species classifier is ran
PACMdetections <- PACMdetections[(PACMdetections$SPECIES_CODE != 'NBHF'), ]

write.csv(PACMdetections, "~/GitHub/ADRIFT-PACM/PACM Worksheets/PASCAL/SWFSC_20240520_DETECTIONDATA.csv", row.names=FALSE)
```