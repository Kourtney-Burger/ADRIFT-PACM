...
allDetections <- subset(allDetections, DriftName != 'PASCAL_014')
# Subset PACM fields
...
## Subset Project
```{r}
detections <- detections[str_detect(detections$DriftName, "CCES"),]
```
...
# Read in metadata
metadata <- read.csv(here::here('PACM Worksheets/CCES/SWFSC_CCES_20240617_METADATA.csv'))
...
for (i in 1:nrow(SampMetadata)) {
  if (SampMetadata$Ratio[i] == '1') {
    SampMetadata$ANALYSIS_PERIOD_EFFORT_SECONDS[i] <- 3600 #This is continuous data, i.e. 3600 seconds = 1 hour
  }
  else {
...
for (i in 1:nrow(PACMdetections)) {
  if (PACMdetections$SPECIES_CODE[i] == "NBHF") {
    ...
    if (PACMdetections$ANALYSIS_SAMPLING_RATE_HZ[i] <= 288000) {
      PACMdetections$MAX_ANALYSIS_FREQUENCY_RANGE_HZ[i] <- PACMdetections$ANALYSIS_SAMPLING_RATE_HZ[i]
    }
    else {
      PACMdetections$MAX_ANALYSIS_FREQUENCY_RANGE_HZ[i] <- "384000"
    }
...
```{r}
for (i in 1:nrow(PACMdetections)) {
  if (PACMdetections$SPECIES_CODE[i] == "GOBW" || PACMdetections$SPECIES_CODE[i] == "BB" || PACMdetections$SPECIES_CODE[i] == "BW43" || PACMdetections$SPECIES_CODE[i] == "BWC" || PACMdetections$SPECIES_CODE[i] == "MC" || PACMdetections$SPECIES_CODE[i] == "MS" || PACMdetections$SPECIES_CODE[i] == "UNME" || PACMdetections$SPECIES_CODE[i] == "BWBB" || PACMdetections$SPECIES_CODE[i] == "BWMC" || PACMdetections$SPECIES_CODE[i] == "BWMS") { 
    PACMdetections$ACOUSTIC_PRESENCE[i] <- "D"
...
write.csv(PACMdetections, "~/GitHub/ADRIFT-PACM/SWFSC_CCES_20240814_DETECTIONDATA.csv", row.names=FALSE)
```