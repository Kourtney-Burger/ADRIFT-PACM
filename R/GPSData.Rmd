...
<!-- rownames(deployDetails) <- 1:nrow(deployDetails) -->
<!-- deployDetails <- deployDetails[-c(9, 24, 27, 32), ] -->
<!-- ``` -->

# Clean and subset data
## Subset data
```{r}
PACM_GPS <- subset(PACM_GPS, select = c('DriftName', 'UTC', 'Latitude', 'Longitude'))

# Select project
PACM_GPS <- subset(PACM_GPS, grepl("ADRIFT", DriftName))

# For pascal only, remove PASCAL 14 - we did not analyze this drift
# PACM_GPS <- subset(PACM_GPS, DriftName != 'PASCAL_014')
```
...
PACM_GPS$UNIQUE_ID = as.factor(PACM_GPS$UNIQUE_ID)
levels(PACM_GPS$UNIQUE_ID) = metaFactorName
```

## Correct Date/Time
```{r}
PACM_GPS$DATETIME <- posixToText(as.POSIXct(PACM_GPS$DATETIME, format = "%Y-%m-%d %H:%M:%S", tz='UTC'))
for (i in 1:nrow(PACM_GPS)) {
  PACM_GPS$DATETIME[i] <- paste0(PACM_GPS$DATETIME[i],'Z')
}
```

# Export GPS Data
```{r}
write.csv(PACM_GPS, "~/GitHub/ADRIFT-PACM/PACM Worksheets/ADRIFT/SWFSC_ADRIFT_20240617_GPSDATA.csv", row.names=FALSE)
```
